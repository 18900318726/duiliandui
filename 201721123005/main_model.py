#训练、评估和推断。由于模型已经搭建好，所以这里的代码块，
# 主要是根据TensorFlow的语法，定义图、会话、占位符，然后传入数据运行会话。
# 附带实现的功能包括保存训练模型，训练日志的记录，模型的加载等

#  定义了model类型，里面包括了模型的输入，输出，重载保存，训练等对模型的操作，是整个项目的重点
import tensorflow as tf
import seq2seq_attention   as seq
...
class Model():
    ...
    def __init__(self, train_input_file, train_target_file,
        ...
        if init_train:
            self.train_reader = data.SeqReader(train_input_file, train_target_file, vocab_file, batch_size)#训练
            self.train_reader.start()
            self.train_data = self.train_reader.read()
            self.eval_reader = data.SeqReader(test_input_file, test_target_file,vocab_file, batch_size) #测试
    ...
    def _init_train(self):
        ...
        with self.train_graph.as_default():
            ...
            output = seq.seq2seq(self.train_in_seq, self.train_in_seq_len,
            ...
            self.loss = seq.seq_loss(output, self.train_target_seq,
    ...
    def _init_eval(self):
        ...
        with self.eval_graph.as_default():
            ...
            self.eval_output = seq.seq2seq(self.eval_in_seq,
    ...
    def _init_infer(self):
        ...
        with self.infer_graph.as_default():
            ...
            self.infer_output = seq.seq2seq(self.infer_in_seq,
        ...
        with self.train_graph.as_default():
            ...
            for step in range(start, epochs):
                ...
                if step % self.save_step == 0:
                    ...
                    input_text = data.decode_text(in_seq[sid],self.eval_reader.vocabs)
                    output_text = data.decode_text(output[sid],
                            self.train_reader.vocabs)
                    target_text = data.decode_text(target_seq[sid],
        ...
        with self.eval_graph.as_default():
            ...
            for step in range(0, self.eval_reader.data_size):
                ...
                for i in range(len(outputs)):
                    output = outputs[i]
                    target = target_seq[i]
                    output_text = data.decode_text(output,
                            self.eval_reader.vocabs).split(' ')
                    target_text = data.decode_text(target[1:],
                            self.eval_reader.vocabs).split(' ')
                    ...
                    if random.randint(1, prob) == 1:
                        print('====================')
                        input_text = data.decode_text(in_seq[i],
                                self.eval_reader.vocabs)
                        print('src:' + input_text)
                        print('output: ' + ' '.join(output_text))
                        print('target: ' + ' '.join(target_text))
            return eval.compute_bleu(target_results, output_results)[0] * 100
    ...
    def infer(self, text):
        ...
        with self.infer_graph.as_default():
            in_seq = data.encode_text(text.split(' ') + ['</s>',],
            ...
            output = outputs[0]
            output_text = data.decode_text(output, self.infer_vocabs)
            return output_text
